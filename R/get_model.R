...
get_model <- function(BCS_data = BCS_data) {
  ...
  BCS_data <- BCS_data %>% 
    dplyr::filter(!NET_YEAR %in% c("2013-36", "2014-35", "2016-33", "2017-2", "2023-14"))
  scaled_data <- BCS_data %>% 
    dplyr::mutate_at(vars(c(6:23)), scale)
  
  # Create a mixed-effects model to estimate the median x and z values for the BCS data with station as a random effect
  bcs_x <- lme4::lmer(median.x ~ 0 + NET_NUMBER:YEAR + (1|STATION), data = scaled_data)
  bcs_z <- lme4::lmer(median.z ~ 0 + NET_NUMBER:YEAR + (1|STATION), data = scaled_data) 
