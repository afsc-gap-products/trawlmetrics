...
get_bottom_data <- function(channel = NULL, contact_dat = contact_dat) {
  ...
  BCS_data <- allcontacts |>
    dplyr::filter(DATE_TIME >= ONBOTTOM, 
                  DATE_TIME <= OFFBOTTOM)
  
  xstat <- BCS_data |> 
    dplyr::group_by(HAUL_ID) |> 
    dplyr::summarize(min = min(X_AXIS),
  ...
  ystat <- BCS_data |> 
    dplyr::group_by(HAUL_ID) |> 
    dplyr::summarize(min = min(Y_AXIS),
  ...
  zstat <- BCS_data |> 
    dplyr::group_by(HAUL_ID) |> 
    dplyr::summarize(min = min(Z_AXIS),
  ...
  zstat <- zstat |> 
    dplyr::rename(min.z = min, q1.z = q1, median.z = median, mean.z = mean, q3.z = q3, max.z = max)
