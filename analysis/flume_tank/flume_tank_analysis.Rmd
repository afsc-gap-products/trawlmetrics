...
The headline height of PNE model was shorter than typically observed in the field (\@ref(fig:field_spread_to_height_pne)), although the slope of the relationship between height and spread suggests changes in geometry resembled those observed in the field. 
...
Future  standard bridle design that has been used on AFSC bottom trawl surveys with a split bridle design featuring separate ground gear and net-bridle connection segments that are connected by a half-bobbin and delta plate. The performance of these bridle designs were tested by comparing the performance of 83-112 and PNE nets with standard rigging to performance using a split-bridle design.
...
```{r speed_v_bridle_angle, include = TRUE, warning = FALSE, fig.cap = "Figure 5. Towing speed versus bridle angle in flume tank experiments under different spread treaments and bridles."}
...
```{r spread_diff_map, include = TRUE, warning=FALSE, fig.cap="Figure 10. Estimated median percent difference between lower wing tip spread and upper wing tip spread, by EBS survey grid cell, based on the regression between upper and lower wing tip spread from flume tank experiments."}
...
```{r mouth_area, include = TRUE, warning=FALSE, fig.cap="Figure 11. Mouth area (m2) for the 83-112, Poly Nor'eastern, RACE Trawl with EBS_v2 footrope and RACE trawl with GOA/AI_v6 footrope."}
...
```{r bridle_tension, include = TRUE, warning=FALSE, fig.cap = "Figure 12. Towing speed (knots) versus bridle tension (metric tons) for the 83-112, Poly nor'eastern, RACE trawl with EBS_v2 footrope, and RACE trawl with GOA/AI_v6 footrope. Color denotes the wing tip spread treatment and symbols indicate the bridle used."}
...
```{r catch_vs_spread_field, include = TRUE, warning=FALSE, fig.cap="Figure 14. Observed total catch weight (kg) versus observed upper wing tip spread (m) in meters, by survey. Points denote observations for individual hauls, solid line and shading denote generalized additve model (GAM) fit and 95% confidence intervals between log10-transformed catch weight and measured spread. Text in the upper left corner shows the deviance explained (%) by each GAM."}
...
```{r catch_vs_height, include = TRUE, warning=FALSE, fig.cap="Figure 17. Flume tank catch treatments versus headline opening height for the 83-112 (standard bridle and bridle #2), Poly nor'eastern (standard bridle), RACE trawl with EBS_v2 footrope (bridle #2), and RACE trawl with GOA/AI_v6 footrope (bridle #2). Symbols (circles/triagles) denote the bridle configuration."}
p1 <- catch_trials |>
...
```{r bb_ctd_spread, include = TRUE, warning=FALSE, fig.cap = "Figure 18. Upper wing tip and lower wing tip spread (m) for flume tank benthic bag (BB), CTD, and spread sensor (Spd) treatments conducted using the RACE trawl with GOA/AI_v6 footrope."}
...
```{r bb_ctd_height, include = TRUE, warning=FALSE, fig.cap = "Figure 19. Headline height (m) for flume tank benthic bag (BB), CTD, and spread sensor (Spd) treatments conducted using the RACE trawl with GOA/AI_v6 footrope."}
