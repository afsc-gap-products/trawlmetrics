...
p_bridle_uws <- ggplot() +
  ...
  scale_x_continuous(name = "Standard bridle u. wing spread (m)") +
  scale_y_continuous(name = "Bridle #2 u. wing spread (m)") +
...
p_bridle_hh <- 
  ...
  geom_abline(slope = 1, intercept = 0, linetype = 2) +
  scale_x_continuous(name = "Standard bridle headline height (m)") +
  scale_y_continuous(name = "Bridle #2 headline height (m)") +
  scale_color_manual(name = "Speed (kn)", values = colors_speed) +
  facet_wrap(~trawl, scales = "free") +
  theme_bw() +
...
```{r speed_v_bridle_angle, include = TRUE, warning = FALSE, fig.cap = "Figure 6. Towing speed versus bridle angle in flume tank experiments under different spread treaments and bridles."}
...
```{r catch_vs_spread_field, include = TRUE, warning=FALSE, fig.cap="Figure 16. Observed total catch weight (kg) versus observed upper wing tip spread (m) in meters, by survey. Points denote observations for individual hauls, solid line and shading denote generalized additve model (GAM) fit and 95% confidence intervals between log10-transformed catch weight and measured spread. Text in the upper left corner shows the deviance explained (%) by each GAM."}
