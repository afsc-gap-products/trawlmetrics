---
title: "Preliminary flume tank analysis"
...
The Resource Assessment and Conservation Engineering Division is undertaking survey modernization efforts to improve upon historical survey data products, streamline survey operations, reduce operational costs, and increase survey design flexibility. A major focus of survey design efforts is to replace existing survey research trawls with a cost effective design that uses easy to source components and provides consistent sampling efficiency for commercially important species across the typical range of operating conditions that surveys encounter. To support gear redesign, the Alaska Fisheries Science Center's Research Fishing Gear Program and Groundfish Assessment Program conducted experiments at the Flume Tank at the Marine Institute of Memorial University of Newfoundland (St. John's, Newfoundland and Labrador, Canada) from January 13-17, 2025.

Flume tank experiments collected baseline performance information from 1:8 scale models of the 83-112 trawl that has been used in the eastern Bering Sea shelf and northern Bering Sea surveys since 1982 and Poly Nor'eastern trawl (PNE) that has been used on Gulf of Alaska and Aleutian Islands bottom trawl surveys since 1980 (Stauffer, 2003). The performance of these trawl designs had not been previously evaluated in a flume tank. Flume tank experiments also evaluated the performance of 1:8 scale prototype trawl and footrope designs being considered to replace the 83-112 and PNE. The performance of model trawls were evaluated across a range of towing speeds, wing spreads, and catch levels that were considered to be representative conditions encountered in the field. The results of the flume tank experiments will inform the next phases of gear redesign.

# Experimental design and results

## Baseline: Poly nor'eastern
...
The headline height of PNE model was shorter than typically observed in the field (Fig. 1), although the slope of the relationship between height and spread suggests changes in geometry resembled those observed in the field. 
...
Future gear may include a split bridle that has separate ground gear and net-bridle connection segments. We compared the geometry of the 83-112 and PNE nets with standard rigging to performance using a split-bridle design. The upper wing tip spread did not differ between bridle for either net. Bridle #2 resulted in a lower headline height for both the 83-112 and PNE.

```{r compare_bridles, include = TRUE, warning=FALSE, fig.cap="Figure 3. Upper wing tip spread and headline height for the 83-112 and Poly'noreastern for the standard bridle versus bridle #2. Colors denote towing speed and the dashed vertical line denotes the 1:1 line. "}
compare_bridles <-
  ...
  dplyr::filter(footrope %in% c("83-112", "PNE"), 
                ...
                trial > 0,
                !benthic_bag) |>
  dplyr::group_by(towing_speed_kn,
                  bridles,
                  trawl,
                  spread_treatment) |>
  dplyr::summarise(door_m = mean(door_m, na.rm = TRUE),
                   spread_u_wing_m = mean(spread_u_wing_m, na.rm = TRUE),
                   spread_l_wing_m = mean(spread_l_wing_m, na.rm = TRUE),
                   opening_headline_m = mean(opening_headline_m, na.rm = TRUE),
                   mouth_area_m2 = mean(mouth_area_m2, na.rm = TRUE),
                   mouth_drag_kgf_m2 = mean(mouth_drag_kgf_m2, na.rm = TRUE), 
                   bridle_angle_deg = mean(bridle_angle_deg, na.rm = TRUE),
                   bridle_tension_total_t = mean(bridle_tension_total_t, na.rm = TRUE)) |>
  tidyr::pivot_wider(
    values_from = 
      c(
        "door_m", 
        ...
        'mouth_drag_kgf_m2',
        "bridle_angle_deg",
        "bridle_tension_total_t"
      ),
    names_from = "bridles"
  )

p_bridle_uws <- ggplot() +
  geom_point(data = compare_bridles,
             mapping = 
               aes(
                 x = spread_u_wing_m_standard, 
                 y = spread_u_wing_m_2,
                 color = format(towing_speed_kn, nsmall = 1),
               )
  ) +
  geom_abline(slope = 1, intercept = 0, linetype = 2) +
  scale_x_continuous(name = "U. wing - Standard (m)") +
  scale_y_continuous(name = "U. wing - #2 (m)") +
  scale_color_manual(name = "Speed (kn)", values = colors_speed) +
  facet_wrap(~trawl, scales = "free") +
  theme_bw() +
...
p_bridle_hh <- 
  ...
  geom_point(data = compare_bridles,
             mapping = 
               aes(
                 x = opening_headline_m_standard, 
                 y = opening_headline_m_2,
                 color = format(towing_speed_kn, nsmall = 1),
               )
  ) +
  geom_abline(slope = 1, intercept = 0, linetype = 2) +
  scale_x_continuous(name = "Height - Standard (m)") +
  scale_y_continuous(name = "Height - #2 (m)") +
  scale_color_manual(name = "Speed (kn)", values = colors_speed) +
  facet_wrap(~trawl, scales = "free") +
  theme_bw() +
...
We compared changes in the ratio between upper wing tip spread and headline height across a range of spreads typically observed on GOA/AI surveys (14, 16, 17 m) and five towing speeds (2-4 knots) that were intended to simulate effects of currents.
...
```{r race_v_pne, include = TRUE, warning=FALSE, fig.cap="Figure 4. Ratio between upper wing tip spread and headline height for the Poly nor'eastern and RACE Trawl with GOA/AI_v6 footrope. Circles denote observations, solid lines and shading respectively show linear regression model fits and 95% confidence intervals."}
...
```{r race_vs_83112, include = TRUE, warning=FALSE, fig.cap="Figure 5. Ratio between upper wing tip spread and headline height for the 83-112 and RACE Trawl with GOA/AI_v6 footrope. Circles denote observations, solid lines and shading respectively show linear regression model fits and 95% confidence intervals."}
p2 <- 
...
```{r speed_v_bridle_angle, include = TRUE, warning = FALSE, fig.cap = "Figure 6. Towing speed versus bridle angle in flume tank experiments under different spread treatments and bridles."}

dat_angle_speed <- trawlmetrics::flume_tank |> 
  dplyr::filter(footrope %in% c("83-112", "PNE", "EBS_v2", "GOA/AI_v6"), 
                catch == "empty",
                pulling_point_elevation_mm <= 200,
                additional_floatation_kg == 0, 
...
p_angle_speed <- ggplot(data = dat_angle_speed,
       mapping = aes(x = towing_speed_kn, 
                     y = bridle_angle_deg, 
  ...
  scale_color_manual(name = "Spread (m)", values = color_spread_all) +
  scale_x_continuous(name = "Speed (kts)") +
...
```{r uwt_spread_angle_of_attack, include = TRUE, warning = FALSE, fig.cap="Figure 7. Upper wing tip spread versus angle of attack for the 83-112, PNE, RACE trawl with EBS_v2, and RACE trawl with GOA/AI_v6 footropes. Circles denote individual observations, solid line and shading denote a GAM fitted mean +/- two standard errors. Dashed horizontal lines denote the target 18-21 degree angle of attack."}
uwt_aoa <- trawlmetrics::flume_tank |> 
  dplyr::filter(footrope %in% c("83-112", "PNE", "EBS_v2", "GOA/AI_v6"), 
                catch == "empty",
                pulling_point_elevation_mm <= 200,
                additional_floatation_kg == 0, 
...
p_spread_aoa <- ggplot() +
  ...
  scale_color_manual(name = "Trawl", values = colors_trawl_footrope) +
...
```{r bridle_angle_analysis, include = TRUE, warning = FALSE}
...
sd_bridle_angle_83112 <- sd(bridle_angle_83112$EST_BRIDLE_ANGLE)
sd_bridle_angle_pne <- sd(bridle_angle_pne$EST_BRIDLE_ANGLE)
...
gam_angle_depth_pne_de <- 
  round(
    ...
    1)
```
Door spread historically has not been measured during regular AFSC bottom trawl survey operations. Therefore, flume tank observations provided a unique opportunity to estimate bridle of attack that may have been observed during actual bottom trawl survey operations in the EBS, NBS, GOA, and AI. Results suggest that `r pct_hauls_18_21_83112`% of 83-112 hauls in the EBS/NBS and `r pct_hauls_18_21_pne`% of PNE hauls in the GOA and AI were predicted to have a bridle angle of attack within the 18-21 degree range.

```{r survey_bridle_angle_plot, include = TRUE, warning=FALSE, fig.cap="Figure 8. Frequency of estimated bridle angle of attack (degrees) for survey hauls based on predictions from generalized additive models fitted between flume tank upper wing tip spread and bridle angle of attack, by survey. Dashed vertical lines denote the 'ideal' angle of attack range (18 to 21 degrees)."}
p_bridle_angle_pne <-
  ggplot() +
  geom_histogram(
...
Survey observations have shown that upper wing tip spread varies with depth, which suggests bridle angle of attack also varies with depth. We fitted generalized additive models between bottom depth and estimated bridle angle of attack for the 83-112 and PNE to evaluate the sensitivity of each gear to variation in bottom depth. Model fits suggest that the bridle angle of attack for the 83-112 is more sensitive to variation in bottom depth than the PNE, as shown by the higher deviance explained (`r gam_angle_depth_83112_de`% for the 83-112, `r gam_angle_depth_pne_de`% for the PNE) and the 83-112's more rapid rate of change in angle of attack within the overlapping bottom depth range for the gears (11-205 m).

```{r bridle_angle_depth, include = TRUE, warning=FALSE, fig.cap="Figure 9. Bottom depth (meters) versus bridle angle of attack (degrees) for the 83-112 and Poly nor'eastern from 2013-2024 bottom trawl surveys. Points denote estimates for individual hauls. Solid lines and shading show generalized additive models fitted mean +/- two standard errors, and deviance explained (DE; %) is shown for each model. Dashed horizontal lines denote the 'ideal' angle of attack range (18 to 21 degrees)."}
p_depth_v_angle <- 
  ggplot() +
  geom_point(
...
If flume tank observations reflect trawl geometry in the field, lower wing tip spread would be 3.8-16.8% wider than upper wing tip spread based on the flume tank regression for the 83-112.

```{r lower_wing_hist_83112, include = TRUE, warning=FALSE, fig.cap="Figure 11. Estimated percent difference between lower wing tip spread and upper wing tip spread based on the flume tank regression for the 83-112, by year. Positive values indicate the estimated lower wing tip spread is wider than the upper wing tip spread."}
...
print(p_spread_diff_hist)
```

```{r spread_diff_map, include = TRUE, warning=FALSE, fig.cap="Figure 12. Estimated median percent difference between lower wing tip spread and upper wing tip spread, by EBS survey grid cell, based on the regression between upper and lower wing tip spread from flume tank experiments."}
p_spread_diff_map <- ggplot() +
...
print(p_spread_diff_map)
```

Spatial variation in upper wing tip spread across the EBS shelf survey area suggests the difference between upper and lower wing tip spread would vary spatially in the EBS, with the lowest differences in shallower areas that typically have narrower spreads and larger differences in deeper areas that typically have wider spreads.


## Mouth area

```{r mouth_area, include = TRUE, warning=FALSE, fig.cap="Figure 13. Mouth area (m2) for the 83-112, Poly Nor'eastern, RACE Trawl with EBS_v2 footrope and RACE trawl with GOA/AI_v6 footrope."}
tension_dat <- trawlmetrics::flume_tank |> 
  dplyr::filter(footrope %in% c("83-112", "PNE", "EBS_v2", "GOA/AI_v6"), 
                catch == "empty",
                pulling_point_elevation_mm <= 200,
                additional_floatation_kg == 0, 
...
lm_mouth_area_83112 <- lm(mouth_area_m2 ~ towing_speed_kn, data = dplyr::filter(tension_dat, trawl == "83-112"))
lm_mouth_area_pne <- lm(mouth_area_m2 ~ towing_speed_kn, data = dplyr::filter(tension_dat, trawl == "PNE"))
lm_mouth_area_race_ebs <- lm(mouth_area_m2 ~ towing_speed_kn, 
                        data = dplyr::filter(tension_dat, trawl == "RACE", footrope == "EBS_v2"))
lm_mouth_area_race_goaai <- lm(mouth_area_m2 ~ towing_speed_kn, 
                          data = dplyr::filter(tension_dat, trawl == "RACE", footrope == "GOA/AI_v6"))

lm_mouth_area_coef <- data.frame(slope = c(lm_mouth_area_83112$coefficients[[2]],
                                      lm_mouth_area_pne$coefficient[[2]],
                                      lm_mouth_area_race_ebs$coefficient[[2]],
                                      lm_mouth_area_race_goaai$coefficient[[2]]),
                            intercept = 
                              c(lm_mouth_area_83112$coefficients[[1]],
                                lm_mouth_area_pne$coefficient[[1]],
                                lm_mouth_area_race_ebs$coefficient[[1]],
                                lm_mouth_area_race_goaai$coefficient[[1]]),
                            trawl = c("83-112", "PNE", "RACE", "RACE"),
                            footrope = c("83-112", "PNE", "EBS_v2", "GOA/AI_v6"))
p_spd_mouth_area <- 
  ggplot() +
  ...
  scale_color_manual(name = "Spread", values = color_spread_all) +
  scale_shape(name = "Bridles", solid = FALSE) +
  facet_wrap(~paste0(trawl, " (", footrope, ")")) +
  theme_bw()

print(p_spd_mouth_area)
```
The 83-112, RACE with EBS_v2 footrope, and RACE trawl with GOA/AI_v6 footrope showed similar rates of changes in mouth area across changes in towing speed, based on the slopes regressions between towing speed and mouth area. The rate of change in PNE mouth area with towing speed differed among spread treatments.

## Trawl drag and bridle tension

Concurrent with trawl design changes, RACE bottom trawl will transition to a new trawl door design to ensure a more consistent trawl geometry across survey conditions and reduce equipment costs through the use of replaceable door shoes, i.e., bolt-on plates that attach to the portion of the door that comes into contact with the seafloor. To inform the selection of new trawl doors, bridle tension measurements were collected during flume tank trials to estimate the drag of the trawl under a range of trawl speeds. Measurements were collected from trials conducted with the Poly Nor'eastern and RACE trawl, but not the 83-112 because of sensor malfunctions.

```{r bridle_tension, include = TRUE, warning=FALSE, fig.cap = "Figure 14. Towing speed (knots) versus bridle tension (metric tons) for the 83-112, Poly nor'eastern, RACE trawl with EBS_v2 footrope, and RACE trawl with GOA/AI_v6 footrope. Color denotes the wing tip spread treatment and symbols indicate the bridle used."}
lm_tension_83112 <- lm(bridle_tension_total_t ~ towing_speed_kn, data = dplyr::filter(tension_dat, trawl == "83-112"))
lm_tension_pne <- lm(bridle_tension_total_t ~ towing_speed_kn, data = dplyr::filter(tension_dat, trawl == "PNE"))
lm_tension_race_ebs <- lm(bridle_tension_total_t ~ towing_speed_kn, 
                         data = dplyr::filter(tension_dat, trawl == "RACE", footrope == "EBS_v2"))
lm_tension_race_goaai <- lm(bridle_tension_total_t ~ towing_speed_kn, 
                          data = dplyr::filter(tension_dat, trawl == "RACE", footrope == "GOA/AI_v6"))

lm_tension_coef <- data.frame(slope = c(lm_tension_83112$coefficients[[2]],
                                     lm_tension_pne$coefficient[[2]],
                                     lm_tension_race_ebs$coefficient[[2]],
                                     lm_tension_race_goaai$coefficient[[2]]),
                           intercept = 
                             c(lm_tension_83112$coefficients[[1]],
                               lm_tension_pne$coefficient[[1]],
                               lm_tension_race_ebs$coefficient[[1]],
                               lm_tension_race_goaai$coefficient[[1]]),
                           trawl = c("83-112", "PNE", "RACE", "RACE"),
                           footrope = c("83-112", "PNE", "EBS_v2", "GOA/AI_v6"))
p_spd_tension <- 
  ggplot() +
  geom_smooth(data = tension_dat,
              mapping = aes(x = towing_speed_kn,
                            y = bridle_tension_total_t),
              method = 'lm',
              color = "black") +
  geom_point(data = tension_dat,
             mapping = aes(x = towing_speed_kn,
                           y = bridle_tension_total_t,
                           color = format(spread_treatment, nsmall = 1),
                           shape = bridles),
             size = rel(2.5)) +
  geom_text(data = lm_tension_coef,
            mapping =
              aes(x = 3,
                  y = 8.25,
                  label = paste0("y=", format(intercept, nsmall = 1, digits = 3), " + ", format(slope, nsmall = 1,digits = 3), "x"))) +
  scale_x_continuous(name = "Towing speed (kn)") +
  scale_y_continuous(name = "Total bridle tension (t)") +
  scale_color_manual(name = "Spread", values = color_spread_all) +
  scale_shape(name = "Bridles", solid = FALSE) +
  facet_wrap(~paste0(trawl, " (", footrope, ")")) +
  theme_bw()
...
```{r trawl_drag, include = TRUE, warning=FALSE, fig.cap = "Figure 15. Towing speed (knots) versus mouth drag (kilogram-force/m2) for the 83-112, Poly nor'eastern, RACE trawl with EBS_v2 footrope, and RACE trawl with GOA/AI_v6 footrope. Color denotes the wing tip spread treatment and symbols indicate the bridle (standard or bridle #2)."}
lm_force_83112 <- lm(mouth_drag_kgf_m2 ~ towing_speed_kn, data = dplyr::filter(tension_dat, trawl == "83-112"))
lm_force_pne <- lm(mouth_drag_kgf_m2 ~ towing_speed_kn, data = dplyr::filter(tension_dat, trawl == "PNE"))
lm_force_race_ebs <- lm(mouth_drag_kgf_m2 ~ towing_speed_kn, 
                          data = dplyr::filter(tension_dat, trawl == "RACE", footrope == "EBS_v2"))
lm_force_race_goaai <- lm(mouth_drag_kgf_m2 ~ towing_speed_kn, 
                            data = dplyr::filter(tension_dat, trawl == "RACE", footrope == "GOA/AI_v6"))

lm_force_coef <- data.frame(slope = c(lm_force_83112$coefficients[[2]],
                                        lm_force_pne$coefficient[[2]],
                                        lm_force_race_ebs$coefficient[[2]],
                                        lm_force_race_goaai$coefficient[[2]]),
                              intercept = 
                                c(lm_force_83112$coefficients[[1]],
                                  lm_force_pne$coefficient[[1]],
                                  lm_force_race_ebs$coefficient[[1]],
                                  lm_force_race_goaai$coefficient[[1]]),
                              trawl = c("83-112", "PNE", "RACE", "RACE"),
                              footrope = c("83-112", "PNE", "EBS_v2", "GOA/AI_v6"))
p_spd_force <- 
  ggplot() +
  ...
  geom_point(data = dat,
             mapping = aes(x = towing_speed_kn,
                           ...
                           shape = bridles),
             size = rel(2.5)) +
  geom_text(data = lm_force_coef,
            mapping =
              aes(x = 3,
                  y = 150,
                  label = paste0("y=", format(intercept, nsmall = 1, digits = 3), " + ", format(slope, nsmall = 1,digits = 3), "x"))) +
  scale_x_continuous(name = "Towing speed (kn)") +
  scale_y_continuous(name = expression('Mouth drag (kgf'%.%m^-2*')')) +
  scale_color_manual(name = "Spread", values = color_spread_all) +
  scale_shape(name = "Bridles", solid = FALSE) +
  facet_wrap(~paste0(trawl, " (", footrope, ")")) +
  theme_bw()
...
```{r catch_vs_spread_field, include = TRUE, warning=FALSE, fig.cap="Figure 16. Observed total catch weight (kg) versus observed upper wing tip spread (m) in meters, by survey. Points denote observations for individual hauls, solid line and shading denote generalized additive model (GAM) fit and 95% confidence intervals between log10-transformed catch weight and measured spread. Text in the upper left corner shows the deviance explained (%) by each GAM."}
...
```{r catch_vs_spread, include = TRUE, warning=FALSE, fig.cap="Figure 18. Flume tank catch treatments versus upper and lower wingtip spread for the 83-112 (standard bridle and bridle #2), Poly nor'eastern (standard bridle), RACE trawl with EBS_v2 footrope (bridle #2), and RACE trawl with GOA/AI_v6 footrope (bridle #2). Symbols (circles/triangles) denote the bridle configuration and colors denote the measurement (upper or lower wing)."}
...
```{r catch_vs_height, include = TRUE, warning=FALSE, fig.cap="Figure 19. Flume tank catch treatments versus headline opening height for the 83-112 (standard bridle and bridle #2), Poly nor'eastern (standard bridle), RACE trawl with EBS_v2 footrope (bridle #2), and RACE trawl with GOA/AI_v6 footrope (bridle #2). Symbols (circles/triangles) denote the bridle configuration."}
p1 <- catch_trials |>
...
```{r bb_ctd_spread, include = TRUE, warning=FALSE, fig.cap = "Figure 20. Upper wing tip and lower wing tip spread (m) for flume tank benthic bag (BB), CTD, and spread sensor (Spd) treatments conducted using the RACE trawl with GOA/AI_v6 footrope."}
bb_ctd_dat <- trawlmetrics::flume_tank |> 
  dplyr::filter(trial %in% c(141, 144, 145))

bb_ctd_dat$bb_ctd <- c("None", "BB", "BB/CTD/Spd")
bb_ctd_dat$bb_ctd <- factor(bb_ctd_dat$bb_ctd, levels = c("None", "BB", "BB/CTD/Spd"))
...
```{r bb_ctd_height, include = TRUE, warning=FALSE, fig.cap = "Figure 21. Headline height (m) for flume tank benthic bag (BB), CTD, and spread sensor (Spd) treatments conducted using the RACE trawl with GOA/AI_v6 footrope."}
...
RACE bottom trawl surveys use a standard scope table to determine how much wire to pay out at a given depth. The scope table helps to ensure the trawl reaches and remains on bottom during standardized survey hauls. The amount of wire required to reach bottom and sink time can vary depending on current speeds.

For each trawl, we sought to determine the door elevation at which the bridles lifted off bottom to provide a reference value to guide accuracy requirements for a scope table.